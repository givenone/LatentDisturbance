...
type Panel={label:string;videos:string[];trace?:Trace;color:string};
export function SyncedPlayer({panels,fps,nSteps,autoPlay=true}:{panels:Panel[];fps:number;nSteps:number;autoPlay?:boolean}){
 ...
 const [playing,setPlaying]=useState(false),[ready,setReady]=useState(false),[error,setError]=useState(''),[speed,setSpeed]=useState(1);
 ...
 const play=async()=>{
  if(!visible.current||pending.current||!wanted.current||dragging.current||!alive.current||all().some(v=>v.readyState<2||v.seeking))return;
 ...
 const markReady=()=>{const usable=all().length===panels.reduce((n,p)=>n+p.videos.length,0)&&all().every(v=>v.readyState>=2);setReady(usable);if(usable&&wanted.current&&!dragging.current)void play();};
