 ...
 return <div className="demo-shell integrated-demo">
  ...
  <div className="demo-workspace">
   ...
   <div className="demo-explorer">
    ...
    <div className="latent-panel"><div className="figure-headings"><h3>World Model Latent Space</h3><h3>Uncertainty Set of Latent Dynamics</h3></div><div className="latent-scene">{selected&&<Drawing kind="scene" action={action} replay={replay}/>}
    {selected&&outcomes.map((o,i)=><div key={o.id}><button className={`outcome-point ${o.id===outcome?'selected':''}`} style={{left:`${50+outcomePoint(action,o.id)[0]*50}%`,top:`${(outcomePoint(action,o.id)[1]*300+45)/4}%`}} onClick={()=>setOutcome(o.id)} aria-label={`Show ${o.label} outcome`} aria-pressed={o.id===outcome}/><button className="outcome-label" style={{left:`${outcomeLabels[i][0]/8}%`,top:`${outcomeLabels[i][1]/4}%`,color:o.color}} onClick={()=>setOutcome(o.id)} aria-pressed={o.id===outcome}>{o.short}</button></div>)}
    </div><div className="schematic-caption">{selected&&<button onClick={()=>setReplay(n=>n+1)} aria-label="Replay latent-space animation">↻ Replay</button>}<span className="sr-only demo-explanation" aria-live="polite">ACTION {action} / {chosen.label.toUpperCase()}</span></div>
   </div>
   <div className="demo-players"><div className="current-state-player"><h3>Current state</h3><VideoPlayer key={`${action}-${playRequest}`} src={demo.original(action)} autoPlay={selected} loop={false} playbackRate={3} onEnded={()=>{if(selected)setCurrentDone(true);}} label="Current state"/></div><div className="imagined-state-player"><h3>WM imaginations</h3><VideoPlayer key={`${demo.video(action,outcome)}-${playRequest}-${currentDone}`} src={demo.video(action,outcome)} label={`Action ${action}, ${chosen.short}`} color={chosen.color} autoPlay={selected&&currentDone} loop={false}/><div className="imagination-selector" aria-label="Select WM imagination">{(['nominal','ours','ood'] as const).map(id=>{const option=outcomes.find(o=>o.id===id)!;return <button key={id} onClick={()=>setOutcome(id)} aria-pressed={outcome===id} style={{'--outcome-color':option.color} as React.CSSProperties}>{option.label}</button>;})}</div></div></div>
   </div>
  </div>
 </div>;
}
